...
```{r}
gapminder %>%  
    filter(country %in% c("Bulgaria", "Romania", "Serbia", "Macedonia", "Greece", "Turkey", "Russia")) %>% 
...
gapminder %>% names()
gapminder %>% 
  filter(country %in% c("Bulgaria", "Romania", "Serbia", "Macedonia", "Greece")) %>% 
  ggplot() +
  aes(x = fct_reorder(country, gdpPercap) , y = gdpPercap, fill = country) +
  coord_flip() +
  geom_col() +
  guides(fill = FALSE) + 
   labs(title='Population growth in Eastern European countries between 1952 and 2007',
             subtitle='Population in {round(frame_time,0)}',
             x='Eastern European countries',
             y='Population')+
