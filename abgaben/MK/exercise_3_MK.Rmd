...
```{r}
ranked_by_year %>%
ggplot(aes(x = gdpPercap, y = fct_reorder(country, gdpPercap), fill = country)) +
  geom_col() +
  scale_x_continuous(limits = c(0,40000)) + 
  labs( x = "Gross Domestic Product per Capita",
       y = "",
       subtitle = "Development of GDP per Capita from 1952 to 2007") +
  theme_light() +
  scale_fill_brewer(palette = "Set3") +
  gganimate::transition_reveal(year) +
  labs(title = "Gross Domestic Product per Capita in:{frame_along}")-> animMK

animate(animMK, fps = 1, nframes = 12)
