...
bcr <- gapminder %>% 
  filter(continent == "Europe") %>% 
    ggplot(aes(x = country,y = pop, group = country, fill=country)) +
        ...
        coord_flip(clip="off") +
        hrbrthemes::theme_ipsum(plot_title_size = 32, subtitle_size = 24, base_size = 20) +
        ...
        labs(title='Population growth in european countries
             between 1952 and 2007',
             subtitle='Population in {round(frame_time,0)}',
             x='European countries',
             y='Population')

animate(bcr, nframes = 750, fps = 25, end_pause = 50, width = 1200, height = 900)
```
