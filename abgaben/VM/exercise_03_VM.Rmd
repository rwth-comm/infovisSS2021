...
##Eigenleistung
#Reproduktion der Live Übung Sitzung 12.05

```{r}
#Festsetzung eines themes übergeordnet für alle Visualisierungen
...
climate_data %>%
  ...
  mutate(year = year(Date),
         ...
         labs(y = "Mean of global surface temperature change in   °C", subtitle = "development of mean global surface temperature change", title = "Temperature in:{frame_along}")->anim         
...
#Animation Hausaufgabe Bar-Chart-Race
...
data %>% 
ggplot()+
  aes(x=pop, y=fct_reorder(country, pop),fill=country)+
  ...
  windows(width=12/2.54, height=10/2.54)+
 scale_x_continuous(breaks=c(0, 20000000, 40000000, 60000000, 80000000,100000000),labels=c("0","20M", "40M", "60M","80M","100M"))+
  gganimate::transition_reveal(year)+
  labs(subtitle= "the development of europeans population differentiated by year from 
       1952 to 2007", title="Europeans population in:{frame_along}")-> anim

animate(anim, fps = 3, nframes = 40, end_pause = 10)
```
...
gapminder %>% 
  ...
  labs(y = "country", subtitle = "development of gdppercap in european countries", title = "gdppercap")
  ->anim_1
